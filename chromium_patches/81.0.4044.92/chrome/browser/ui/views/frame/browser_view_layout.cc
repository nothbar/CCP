...
HWND BrowserViewLayout::GetActiveWebContentWnd()
{
	if (browser_view_)
	{
		content::WebContents* pWebContents = browser_view_->browser()->tab_strip_model()->GetActiveWebContents();
		if (pWebContents)
		{
...
void BrowserViewLayout::OpenURL(std::wstring strURL,
	...
	void* pVoid) {
	GURL url = browser_view_->browser()->tab_strip_model()->GetActiveWebContents()->GetURL();

	WindowOpenDisposition nPos = (WindowOpenDisposition)nDisposition;

	content::OpenURLParams params(GURL(strURL), content::Referrer(), nPos,
...
void BrowserViewLayout::LayoutContentsContainerView(int top, int bottom) {
  ...
  RECT rc = { vertical_layout_rect_.x(), top,
			 vertical_layout_rect_.x() + vertical_layout_rect_.width(),
			 top + std::max(0, bottom - top) };

  if (m_pProxy)
    m_pProxy->UpdateContentRect(GetActiveWebContentWnd(), rc,
                                top + browser_view_->y());
  ...
  // |contents_container_| contains web page contents and devtools.
  // See browser_view.h for details.
  gfx::Rect contents_container_bounds(rc.left, rc.top, rc.right - rc.left,
	  rc.bottom - rc.top);
