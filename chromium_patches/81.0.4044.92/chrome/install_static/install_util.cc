...
std::wstring GetUninstallRegistryPath() {
  ...
  // begin Add by TangramTeam
  if (g_strProductName != L"") {
	  if (g_strCompanyPath != L"")
		  result.append(g_strCompanyPath).append(1, L' ');
	  // size_t nSize = wcslen(g_strProductName.c_str());
	  result.append(g_strProductName);
	  // result.append(g_strProductName, nSize);
  }
  else {
	  if (*kCompanyPathName)
...
std::wstring& AppendChromeInstallSubDirectory(const InstallConstants& mode,
	...
	if (g_strConfigFile == L"") {
		...
		g_strConfigFile = m_szBuffer;
		g_strConfigFile.append(L".tangram");
		if (g_strConfigFile != L"") {
			CMarkup mk;
			if (mk.Load(g_strConfigFile.c_str()) && mk.FindNode()) {
				g_strCompanyPath = mk.GetAttrib(_T("companypathname"));
				g_strProductName = mk.GetAttrib(_T("productname"));
				if (g_strProductName == L"")
					g_strProductName = kProductPathName;
			}
		}
	}
	if (g_strProductName != L"") {
		if (g_strCompanyPath != L"") {
			path->append(g_strCompanyPath);
			path->push_back(L'\\');
		}
		// size_t nSize = wcslen(g_strProductName.c_str());
		path->append(g_strProductName);
		// path->append(g_strProductName, nSize);
		if (!include_suffix)
			return *path;
