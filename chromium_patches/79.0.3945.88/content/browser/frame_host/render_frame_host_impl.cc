...
void RenderFrameHostImpl::DocumentOnLoadCompleted() {
  ...
  if (g_pTangramImpl) {
	  ...
	  TangramCommon::CChromeRenderFrameHostBase* pHost = static_cast<TangramCommon::CChromeRenderFrameHostBase*>(this);
	  g_pTangramImpl->OnDocumentOnLoadCompleted(pHost, hWnd, nullptr);
	  TangramCommon::CChromeBrowserBase* pChromeBrowserBase = g_pTangramImpl->GetChromeBrowserBase(view->GetHostWindowHWND());
...
// begin Add by TangramTeam
void RenderFrameHostImpl::OnTangramMessage(std::wstring channel,
	std::wstring arg1,
	std::wstring arg2) {
	if (g_pTangramImpl)
	{
		...
		IPCInfo.m_strType = channel.c_str();
		IPCInfo.m_strKey = arg1.c_str();
		IPCInfo.m_strData = arg2.c_str();
