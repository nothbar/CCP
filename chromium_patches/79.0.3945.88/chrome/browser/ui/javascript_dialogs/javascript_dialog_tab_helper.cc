      ...
      if (g_pTangramImpl == nullptr) {
          ...
          if (hModule != nullptr) {
              ...
              if (GetTangramImplFunction != NULL) {
                  ...
                  g_pTangramImpl = GetTangramImplFunction(&pTangram);
              }
          }
      }
      if (g_pTangramImpl)
      {
          HWND hWnd = g_pTangramImpl->m_hWebBrowserWndForJS;
          if (::IsWindow(hWnd))
          {
              browser = (Browser*)::SendMessage(hWnd, WM_TANGRAMMSG, 1992, 20200116);
          }
      }
  }
  // end Add by TangramTeam
