...
void RecordLaunchModeHistogram(LaunchMode mode) {
  // begin Add by TangramTeam
  if (g_pTangramImpl && g_pTangramImpl->m_nAppType == TANGRAM_APP_BROWSER_ECLIPSE)
	return;
  // end Add by TangramTeam
...
HWND ChromeBrowserFactory::CreateBrowser(HWND hParent, CString strXml) {
	...
	if (profile_) {
		...
		if (b == true) {
			if (g_pTangramImpl && ::IsWindow(hParent))
				g_pTangramImpl->m_hParent = hParent;
			...
			hBrowser = views::HWNDForNativeWindow(browser->window()->GetNativeWindow());
			g_pTangramImpl->m_hParent = NULL;
			g_pTangramImpl->OnSubBrowserWndCreated(hParent, hBrowser);
			browser->window()->Show();
		}
	}
	return hBrowser;
...
bool StartupBrowserCreatorImpl::Launch(Profile* profile,
  ...
  if (g_pTangramImpl && g_pTangramImpl->m_pBrowserFactory) {
	  ChromeBrowserFactory* pImpl =
		  (ChromeBrowserFactory*)g_pTangramImpl->m_pBrowserFactory;
	  if (pImpl && pImpl->profile_ == nullptr)
  ...
  if (OpenApplicationWindow(profile)) {
	...
	// begin Add by TangramTeam
	if ((g_pTangramImpl && g_pTangramImpl->m_nAppType == TANGRAM_APP_BROWSER) || process_startup == false)
		// end Add by TangramTeam
  ...
  {
	  bool bRet = false;
	  if (g_pTangramImpl->m_pTangramDelegate)
		  bRet = g_pTangramImpl->m_pTangramDelegate->BrowserAppStart();
	  if (bRet == false)
		  g_pTangramImpl->BrowserAppStart();
  }
  // end Add by TangramTeam
#endif  // defined(OS_WIN)
