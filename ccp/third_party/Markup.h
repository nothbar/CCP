...
#pragma once
#pragma warning( \
    disable : 4996)  // suppress VS 2005 deprecated function warnings
#ifdef MARKUP_STL
...
#define MCD_RELEASEBUFFER(s, p, n) \
  s.assign(p, n);                  \
  delete[] p
...
#define MCD_STRASSIGN(s, p, n)                       \
  memcpy(s.GetBuffer(n), p, (n) * sizeof(MCD_CHAR)); \
  s.ReleaseBuffer(n);
...
#define MCD_BLDAPPENDN(s, p, n) \
  MCD_PSZNCPY(&pD[nL], p, n);   \
  nL += n
#define MCD_BLDAPPEND(s, p) \
  MCD_PSZCPY(&pD[nL], p);   \
  nL += MCD_PSZLEN(p)
...
class CMarkup {
 public:
  ...
  bool InsertChildElem(MCD_CSTR szName,
  ...
  bool RemoveNode();
  bool SetAttrib(MCD_CSTR szAttrib, MCD_CSTR szValue) {
    return x_SetAttrib(m_iPos, szAttrib, szValue);
  }
  bool SetChildAttrib(MCD_CSTR szAttrib, MCD_CSTR szValue) {
    return x_SetAttrib(m_iPosChild, szAttrib, szValue);
  }
  bool SetAttrib(MCD_CSTR szAttrib, int nValue) {
    return x_SetAttrib(m_iPos, szAttrib, nValue);
  }
  bool SetChildAttrib(MCD_CSTR szAttrib, int nValue) {
    return x_SetAttrib(m_iPosChild, szAttrib, nValue);
  }
  ...
  // Utility
  static bool ReadTextFile(MCD_CSTR szFileName,
                           MCD_STR& strDoc,
                           MCD_STR* pstrError = NULL,
                           int* pnFlags = NULL);
  static bool WriteTextFile(MCD_CSTR szFileName,
                            MCD_STR& strDoc,
                            MCD_STR* pstrError = NULL,
                            int* pnFlags = NULL);
 ...
 protected:
  ...
  struct ElemPos {
    ...
    void SetEndTagLen(int n) {
      nTagLengths = (nTagLengths & EP_STMASK) + (n << EP_STBITS);
    }
    ...
    int nStart;
    int nLength;
    int nTagLengths;  // 22 bits 4MB limit for start tag, 10 bits 1K limit for
                      // end tag
  ...
  struct SavedPosMap {
    ...
    void Release() {
      for (int n = 0; n < SPM_SIZE; ++n)
        if (pTable[n])
          delete[] pTable[n];
      delete[] pTable;
    }
    void AllocMapTable() {
      pTable = new SavedPos*[SPM_SIZE];
      for (int n = 0; n < SPM_SIZE; ++n)
        pTable[n] = NULL;
    }
    int Hash(MCD_PCSZ szName) {
      unsigned int n = 0;
      while (*szName)
        n += (unsigned int)(*szName++);
      return n % SPM_SIZE;
    }
    SavedPos** pTable;
  };
  ...
  struct PosArray {
    ...
    ElemPos& operator[](int n) const {
      return pSegs[n >> PA_SEGBITS][n & PA_SEGMASK];
    }
    ElemPos** pSegs;
    int nSize;
    int nSegs;
  ...
  void x_RemoveNode(int iPosParent,
                    int& iPos,
                    int& nNodeType,
                    int& nNodeOffset,
                    int& nNodeLength);
