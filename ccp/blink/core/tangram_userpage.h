...
class CORE_EXPORT TangramUserpage final : public EventTargetWithInlineData,
									  public DOMWindowClient {
  DEFINE_WRAPPERTYPEINFO();
  USING_GARBAGE_COLLECTED_MIXIN(TangramUserpage);

 public:
  static TangramUserpage* Create(LocalFrame* frame) { return MakeGarbageCollected<TangramUserpage>(frame); }
  static TangramUserpage* Create(LocalFrame* frame, const String& strHandle);
  ...
  DEFINE_ATTRIBUTE_EVENT_LISTENER(UserPageMessageReceived, kTangramuserpage)

  // EventTarget overrides:
  const AtomicString& InterfaceName() const override;
  ExecutionContext* GetExecutionContext() const override;

  explicit TangramUserpage(LocalFrame*);
  TangramUserpage(LocalFrame*, const String& strNodeXml);

  ~TangramUserpage() override;
